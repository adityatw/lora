...
- Quantization: Initially attempted 8-bit via bitsandbytes; disabled due to SCB attribute error
...
- Query: Determines which tokens to attend to
...
## ⚙️ CUDA Pre-Check
Before training, ensure the GPU is available and properly recognized:
...
- Split: 80/20 train/test using Hugging Face’s native .train_test_split() to avoid indexing errors
...
## 🔍 LoRA vs Transfer Learning: Key Difference
...
This experiment demonstrates how LoRA can be effectively applied to question answering tasks using `flan-t5-small`, even on limited hardware like a GTX 1650. Through careful parameter tuning, memory-aware evaluation, and explicit metric tracking via Weights & Biases, we show that meaningful fine-tuning is possible without full-model retraining. The workflow is designed to be reproducible, modular, and adaptable—serving as a scaffold for future schema-aware, agentic AI pipelines.
...
This setup has been tested on a GTX 1650 with limited VRAM, and includes memory-aware strategies to prevent out-of-memory errors during training and evaluation.
